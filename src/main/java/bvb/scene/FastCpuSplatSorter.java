...
public class FastCpuSplatSorter {
    ...
    private int currN = -10;
    boolean bAllocated = false;
    boolean bHasColors = false;
    boolean bHasSizes = false;

    public void allocate(int n, boolean hasColors, boolean hasSizes) 
    {
    	if(!bAllocated || n != currN || hasColors != bHasColors || hasSizes != bHasSizes)     		
    	{
    ...
    public void sortBackToFront(
        ...
        float[] spotSizes, 
        float[] colors, 
        float [] viewAndOffset, boolean hasColors, boolean hasSizes
    ) {
       
    	allocate(N, hasColors, hasSizes);
        ...
        for (int i = 0; i < N; i++) 
        {
            ...
            float depth = vx * viewAndOffset[0] + vy * viewAndOffset[1] + vz * viewAndOffset[2] + viewAndOffset[3];
            //float depth = vz;
